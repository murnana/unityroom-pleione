---
paths:
  - "Packages/com.murnana.unityroom/Editor/**/*.cs"
---

# unityroom Localization

## Rule

All **user-facing strings** (GUI labels, error logs, warning messages, etc.) in `Packages/com.murnana.unityroom/Editor/` must be wrapped with Unity's `L10n.Tr()` to support Japanese localization.

## Requirements

1. Wrap all user-facing strings with `L10n.Tr()`
2. Strings passed to `L10n.Tr()` (`msgid`) must be written in **English**
3. When adding a new string, add a corresponding `msgid` / `msgstr` pair to `Packages/com.murnana.unityroom/Editor/Localization/ja.po`
